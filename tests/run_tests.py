...
AE_dat.calc_AE(omn=1.0,omt=1.0,omnigenous=True)
AE_dat.plot_AE_per_lam()
omnigenous      = AE_dat.ae_tot
AE_dat.calc_AE(omn=1.0,omt=1.0,omnigenous=False)
...
AE_dat.calc_AE(omn=1.0,omt=1.0,omnigenous=False)
slow      = AE_dat.ae_tot
AE_dat.calc_AE_fast(omn=1.0,omt=1.0,omnigenous=False)
fast  = AE_dat.ae_tot
...
####### Interestingly, a difference arises for W7XSC
# First import gist data
file_name='gist_W7XSC.txt'
data = mag_reader.mag_data("gist_files/"+file_name)
data.include_endpoint()
data.refine_profiles(10001)
data.plot_geometry()
...
AE_dat = ae.AE_gist(data,quad=False,lam_res=lam_res)
AE_dat.plot_precession()
AE_dat.calc_AE(omn=1.0,omt=0.0,omnigenous=False)
slow      = AE_dat.ae_tot
AE_dat.calc_AE_fast(omn=1.0,omt=0.0,omnigenous=False)
fast  = AE_dat.ae_tot
# find difference between slow and fast method
print('(slow-fast)/fast = ', (slow-fast)/fast)
...
for omn_idx, omn_val in np.ndenumerate(omn_range):
    AE_dat.calc_AE(omn=omn_range[omn_idx],omt=0.0,omnigenous=True)
...
for omn_idx, omn_val in np.ndenumerate(omn_range):
    ...
    AE_dat.calc_AE_fast(omn=omn_range[omn_idx],omt=0.0,omnigenous=False)
